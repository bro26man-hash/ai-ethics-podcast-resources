...
The fix is specific and concrete:
...
- If the docstring is wrong, does the metric itself need to change — or just the words around it?
...
4. **The intersectionality gap.** Most fairness toolkits measure bias one protected attribute at a time. But race + gender, disability + age, immigration status + religion — these intersections are where the worst abuses happen. A single scalar per attribute can't capture them.
...
- Should fairness metrics be required to report breakdowns for at least the top 3 most-affected group intersections?

---

## Debate 3: The MetricFrame API Design Debate — Who Does the Tool Serve?
...
`MetricFrame` is FairLearn's flagship tool for computing fairness metrics with disaggregation by sensitive features. But it only works with metrics that have the signature `metric(y_true, y_pred)`. Several researchers and contributors want to extend it to support:

- **Dataset-only metrics** (like demographic parity that only needs `y_true`)
- **Streaming metrics** for online fairness monitoring
- **Metrics from other domains** (contextual bandits, cost-sensitive learning)

MiroDudik proposes making all arguments keyword-only and allowing flexible shared sample parameters. The maintainers disagree on whether this flexibility is worth the complexity.
...
## Debate 4: The Missing-Value Contract — Should Fairness Tools Be Strict or Silent?
...
## Debate 5: The SHAP Measurement Problem — Does the Denominator Change the Story?
