...
A volunteer (Hanabi9248) offered to fix both the API docstrings and the explanations with a concrete four-row example where `average_odds_difference = 0` while both `average_abs_odds_difference` and `equalized_odds_difference` equal 1. But after months, no maintainer has merged the fix.
...
- **The gap between research and deployment.** This issue was filed in 2024 and remains open. The toolkit that bridges academic fairness research and real-world deployment has open questions about its own correctness — and a volunteer is waiting for a maintainer to merge a fix.
- **The asymmetry of effort.** One person offered a precise, example-backed fix. The maintainer side has been silent. This pattern — community labor, maintainer inaction — recurs across fairness tooling.
...
## Debate 4: The Documentation Gap — Aequitas #201

**Issue:** [`Add a readme or page on the existing metrics of fairness`](https://github.com/dssg/aequitas/issues/201)
**Repo:** [dssg/aequitas](https://github.com/dssg/aequitas)
**Opened:** July 2024 · **Status:** Still open · **Comments:** 2
**Labels:** `Good First Issue`, `Base`, `Documentation`

### The Issue

Two contributors offered to create better documentation for Aequitas's fairness metrics, noting that the current documentation at `dssg.github.io/aequitas/metrics.html` "is not great in terms of being informative."

While not a dramatic disagreement, this issue reveals a quieter but persistent tension: **tooling that serves non-technical stakeholders must itself be understandable by non-technical stakeholders.** When the documentation for a fairness audit tool is written by and for data scientists, it undermines the tool's stated mission of community accessibility.

### 🎙️ Podcast Talking Points

- **Documentation is a fairness issue.** If the people most affected by algorithmic bias can't understand the tool meant to audit it, the tool serves the auditors, not the audited.
- **The "good first issue" paradox.** Issues labeled as beginner-friendly often remain open because they lack the attention of experienced contributors who could guide them. Documentation is labor that's invisible when done well and obvious when neglected.

---

## 🔑 Cross-Cutting Themes

| Theme | FairLearn #756 | AIF360 #528 | AIF360 #558 | Aequitas #201 |
|-------|----------------|-------------|-------------|---------------|
| Who does this tool serve? | Practitioners vs. researchers | Audit consumers | Audit consumers | Non-technical stakeholders |
| Simplicity vs. power | Core design tension | N/A | Scalar vs. granular | Clarity vs. completeness |
| Backward compatibility | Central concern | N/A | N/A | N/A |
| Trust & correctness | N/A | Metric definition is wrong | Aggregation hides harm | Documentation obscures metrics |
| Maintainer bandwidth | Active debate, 74 comments | Open since 2024, fix offered | Open since 2026, fix offered | Open since 2024, contributors waiting |
| Community labor vs. institutional response | Resolved through compromise | Volunteer fix unmerged | Volunteer fix unmerged | Contributors waiting for assignment |
...
3. **Want to analyze these debates further?** Check out the original issue threads and join the conversation
